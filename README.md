...
From there you can:
- **Add a product** — click "+ Add product", fill in name, category, price, stock, and a description, then upload up to 6 photos from your computer or phone.
  ...
  - **Photo per option** — once you've typed your Options, a small photo-upload box appears for each choice (like each color). Upload a photo for any option you want a different picture for — leave the rest blank if they should just use the product's regular photos. When a customer picks that option, the photo switches automatically.
...
- **Set a warranty** — when adding or editing a product, the "Warranty" field (e.g. "1 Year Warranty") shows on that product's page for customers.
- **Create shipping methods** — click "Shipping" in the sidebar to add options like "Standard" or "Express", each with its own fee and estimated delivery window (e.g. 3–5 days). Customers pick one at checkout, and it's added to their total automatically. If you don't set any up, checkout defaults to free standard shipping.
- **Create discount vouchers** — click "Vouchers" in the sidebar to create codes like `SAVE10` (percentage off) or `WELCOME50` (fixed amount off). You can optionally set a minimum order amount, a usage limit, and an expiry date. Customers enter the code at checkout, and the discount applies automatically to their total — validated on our server, so it can't be tampered with from the browser.
...
**Note on vouchers + PayMongo:** when a customer applies a discount voucher and pays online, PayMongo's checkout page shows one combined line item for the final total rather than itemizing each product — this is because PayMongo doesn't support negative amounts for discount line items. Cash on Delivery orders and your admin Orders tab always show the full itemized breakdown regardless.
